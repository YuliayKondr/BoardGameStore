...
{
    /// <summary>
    /// класс предаставляющий возможность отправки
    /// создан как пример
    /// но в коде реализуется сохранение на диске С:
    /// </summary>
    ...
    public class EmailOrderProcessor : IOrderProcessor
    {
        ...
        public void ProcessOrder(Cart cart, ShippingDetails shippingInfo)
        {
            using (var smtpClient = new SmtpClient())
            {
                ...
                {
                    var subtotal = line.BoardGame.Price * line.Quantity;
                    body.AppendFormat("{0} x {1} (итого: {2:c}",
                        line.Quantity, line.BoardGame.NameGame, subtotal);
                }
